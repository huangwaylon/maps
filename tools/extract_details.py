...
import fetch_place
from fetch_place import fetch,parse            # noqa: F401  (reused as-is)
...
def done_keys():
    ...
    keys=set()
    if not os.path.exists(OUT): return keys
    with open(OUT) as f:
        for line in f:
            line=line.strip()
            if not line: continue
            try: keys.add(json.loads(line)['key'])
            except Exception: pass       # tolerate a torn final line
    return keys
    ...
    for name,hl,gl in LOCALES:
        ...
        for attempt in range(retries):
            try:
                ...
                if doc is None: raise RuntimeError("no preload token / empty RPC")
                rec[name]=parse(doc); last=None; break
...
def run(args):
    ...
    print("%d places, %d already done, %d to fetch (%d workers, %.1f req/s)"
          %(len(places),len(places)-len(todo),len(todo),args.workers,args.rps),flush=True)
    if not todo: return
    ...
    state={"done":0,"fail":0}; t0=time.time()
    ...
    def work(p):
        ...
        with wlock:
            ...
            state["done"]+=1
            if "error" in rec: state["fail"]+=1
            n=state["done"]
            if n%25==0 or n==len(todo):
                el=time.time()-t0; rate=n/el
                print("  %d/%d  fail=%d  %.0fs elapsed  ~%.0fs left  (%.1f places/min)"
                      %(n,len(todo),state["fail"],el,(len(todo)-n)/rate,rate*60),flush=True)
    ...
    print("done: %d in %.0fs (%.1f places/min), %d failures"
          %(state["done"],el,state["done"]/el*60,state["fail"]),flush=True)
